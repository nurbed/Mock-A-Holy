...
inline fixed4 calculateLitPixel(fixed4 texureColor, fixed4 color, fixed3 lighting) : SV_Target
{
	...
	finalPixel.rgb = (texureColor.rgb * lighting * color.rgb) * finalPixel.a;
#endif
...
#define ALPHA_CLIP(alpha, color) clip((alpha * color.a) - _Cutoff);

#else

#define ALPHA_CLIP(alpha, color)
...
#define COLORISE(pixel) pixel.rgb = lerp(pixel.rgb, _OverlayColor.rgb, _OverlayColor.a);
